...
namespace LegoTwin.Character
{
    ...
    public static class HumanoidAvatarBuilder
    {
        ...
        public static bool Build(GameObject root)
        {
            ...
            {
                Debug.LogWarning(
                    $"[HumanoidAvatarBuilder] Avatar 빌드 실패: {root.name}\n" +
                    $"  isValid={avatar.isValid}, isHuman={avatar.isHuman}, 매핑된 본: {humanBones.Count}개");
                return false;
            }

            avatar.name     = $"{root.name}_HumanoidAvatar";
            animator.avatar = avatar;
            Debug.Log($"[HumanoidAvatarBuilder] ✓ {root.name} — isValid={avatar.isValid}, isHuman={avatar.isHuman}, 매핑 본: {humanBones.Count}개");
            return true;
        }
